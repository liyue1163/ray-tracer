/***********************************************************
	
	Starter code for Assignment 3
	
	This file contains the interface and datastructures of the raytracer.  
	Simple traversal and addition code to the datastructures are given to you.

***********************************************************/
#pragma once

#include "util.h"
#include "scene_object.h"
#include "light_source.h"

using namespace std;

class Raytracer {
public:
	// Renders 3D scene to an image given camera and lights setup.
	void render(Camera& camera, Scene& scene, LightList& light_list, Image& image,
		bool aFlag, bool dFlag, bool sFlag, bool gFlag,
		Scene envPlanes = Scene(), vector<Image *> envImages = vector<Image *>(), vector<Image *> textureImages = vector<Image *>());
		
private:

	// Return the color of the ray after intersection and shading, call 
	// this function recursively for reflection and refraction.  
	Color shadeRay(Ray3D& ray, Scene& scene, LightList& light_list, int numReflection,
		Scene& envPlanes, vector<Image *>& envImages, vector<Image *>& textureImages, 
		int sNum, int gNum);

	// Traversal code for the scene, the ray is transformed into 
	// the object space of each node where intersection is performed.
	// return which object it hit if hit anything
	int traverseScene(Scene& scene, Ray3D& ray);

	// After intersection, calculate the color of the ray by shading it
	// with all light sources in the scene.
	void computeShading(Ray3D& ray, Scene& scene, LightList& light_list, int sNum);

	// Precompute the modelToWorld and worldToModel transformations for each
    // object in the scene.
	void computeTransforms(Scene& scene);

	// when eventually hit nothing
	// calculate the color of the ray by map environment to it
	void computeEnvMapping(Ray3D& ray, Scene& envPlanes, vector<Image *>& envImages);

	// given SceneNode the ray intersected and corresponding image
	// compute texture mapping color and set it to ray
	void computeTextureMapping(Ray3D& ray, SceneNode * node, Image * image);
};
