...
struct Image {
	...
	Image(int width, int height, unsigned char *rarray, unsigned char *garray, unsigned char *barray)
	:
	width(width), height(height){
		initPixelBuffer();
		setPixelBuffer('r', rarray);
		setPixelBuffer('g', garray);
		setPixelBuffer('b', barray);
	}
	...
	// set buffer with corresponding color
	void setPixelBuffer(char col, unsigned char * colArray);
	...
	// get color of pixel (i, j) and store it to col
	void getColorAtPixel(int u, int v, Color& col);
...
// convert xyz in world space to uv in texture coordinates
// index:		0: posx; 1:negx; 2:posy; 3:negy; 4:posz; 5:negz
// v: vertical; u:horizontal (u, v are between 0 and 1)
void xyz_to_iuv(double x, double y, double z, int *index, double *u, double *v);

// convert a point in sphere to uv in texture
void xyz_to_uv(Point3D intersection, Point3D center, double * u, double * v);

// create a vector of Image pointers to store environment images.
void createEnvImages(std::vector<Image*>& envImages, std::string file_in_dir, int width, int height);

// create a vector of Image pointers to store texture images
// each file_in_name, width, height corresponds to SceneNode in scene
